...
exports.default = {
    ...
    func: async (_req, res) => {
        if (_req.method === 'OPTIONS')
            return res.send();
        if (_req.method === 'GET') {
            axios_1.default.get('https://gameinternationalization.roblox.com' + _req.url, {
            ...
            axios_1.default.patch('https://gameinternationalization.roblox.com' + _req.url, _req.body, {
                headers: { ..._req.headers, host: 'gameinternationalization.roblox.com' },
            })
                .then((re) => {
                const newheaders = JSON.parse(JSON.stringify(re.headers).split('roblox.com').join('sitetest4.robloxlabs.com'));
